...
using NastriServer.Services;

namespace NastriServer
{
    class Program
    {
        private static ApplicationInstance? application;
        private static NastriOpcServer? server;

        static async Task Main(string[] args)
        {
            Console.WriteLine("=== Server OPC-UA Nastri ===");
            ...
            try
            {
                ...
                {
                    ApplicationName = "Server Nastri",
                    ApplicationType = ApplicationType.Server
                };

                // Crea configurazione programmaticamente (senza file XML)
                var config = new ApplicationConfiguration()
                {
                    ApplicationName = "Server Nastri",
                    ApplicationUri = "urn:localhost:MVLabs:NastriServer",
                    ProductUri = "http://mvlabs.it/NastriServer",
                    ApplicationType = ApplicationType.Server,
                    ...
                    {
                        BaseAddresses = { "opc.tcp://localhost:4841/NastriServer" },
                ...
                // Crea il server
                server = new NastriOpcServer();
                await application.Start(server);

                Console.WriteLine("Server avviato!");
                Console.WriteLine($"Endpoint: opc.tcp://localhost:4841/NastriServer");
